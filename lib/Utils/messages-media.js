...
// Change getMediaKeys to be an async function
async function getMediaKeys(buffer, mediaType) {
    if (!buffer) {
        throw new boom_1.Boom('Cannot derive from empty media key');
    }
    
    console.log('DEBUG: getMediaKeys input:', {
    ...
    if (typeof buffer === 'string') {
        console.log('DEBUG: Converting string to buffer');
        // Remove data URL prefix if present and convert from base64
        if (buffer.startsWith('data:;base64,')) {
    ...
    if (!Buffer.isBuffer(buffer)) {
        console.log('DEBUG: Not a buffer:', typeof buffer, buffer);
        throw new boom_1.Boom('Media key must be a Buffer or base64 string');
    }
    ...
    // For media keys, they should be 32 bytes
    if (buffer.length !== 32) {
        console.log('DEBUG: Buffer length is', buffer.length, 'hashing to get 32 bytes');
    ...
    const info = (0, exports.hkdfInfoKey)(mediaType);
    console.log('DEBUG: Calling hkdf with:', {
        bufferLength: buffer.length,
        length: 112,
        info
    });
    ...
    // AWAIT THE HKDF CALL SINCE IT RETURNS A PROMISE
    const expandedMediaKey = await (0, crypto_1.hkdf)(buffer, 112, { info });
    
    console.log('DEBUG: hkdf returned:', {
        type: typeof expandedMediaKey,
        isBuffer: Buffer.isBuffer(expandedMediaKey),
        value: expandedMediaKey
    });
    
    // Ensure expandedMediaKey is a Buffer
    if (!Buffer.isBuffer(expandedMediaKey)) {
        console.log('DEBUG: hkdf did not return a Buffer:', expandedMediaKey);
        // Try to convert it if it's not a buffer
        if (typeof expandedMediaKey === 'string') {
            expandedMediaKey = Buffer.from(expandedMediaKey, 'hex');
        } else if (Array.isArray(expandedMediaKey)) {
            expandedMediaKey = Buffer.from(expandedMediaKey);
        } else {
            throw new boom_1.Boom('HKDF did not return a Buffer', { data: expandedMediaKey });
        }
    }
    
    if (expandedMediaKey.length < 80) {
        throw new boom_1.Boom(`HKDF returned insufficient bytes: ${expandedMediaKey.length} < 80`);
    }
...
const encryptedStream = async (media, mediaType, { logger, saveOriginalFileIfRequired, opts, isPtt, forceOpus } = {}) => {
    ...
    // DEBUG: Log the mediaKey
    logger?.debug('Generated mediaKey:', mediaKey.toString('hex'));
    
    // AWAIT THE GETMEDIAKEYS CALL AND GET THE RESULT
    const mediaKeysResult = await getMediaKeys(mediaKey, mediaType);
    
    // DEBUG: Log the mediaKeysResult
    logger?.debug('Media keys result:', {
        hasIv: !!mediaKeysResult.iv,
        hasCipherKey: !!mediaKeysResult.cipherKey,
        hasMacKey: !!mediaKeysResult.macKey,
        ivLength: mediaKeysResult.iv?.length,
        cipherKeyLength: mediaKeysResult.cipherKey?.length,
        macKeyLength: mediaKeysResult.macKey?.length
    });
    
    // Destructure AFTER awaiting
    const { cipherKey, iv, macKey } = mediaKeysResult;
    
    if (!cipherKey || !iv || !macKey) {
        throw new boom_1.Boom('Failed to derive media keys', {
            data: { cipherKey, iv, macKey }
        });
    }
